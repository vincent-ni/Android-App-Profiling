...
int GlobalParam::       _NoOpenGLinCUDA = 0; //direct data transfer from CPU to CUDA (1)or thought OpenGL(0)
	...
	for(int i = 0; i< argc; i++)
	{
        ...
        case 'c' + ('u' << 8) + ('d' << 16) + ('a' << 24):
            if(i + 1 < argc)
            {
               ...
               GlobalParam::_CudaDeviceIndex = device;
            }
            break;  
        case 'n' + ('o' << 8) + ('g' << 16) + ('l' << 24):
            GlobalParam::_NoOpenGLinCUDA = 1;
            break;
        default:
            break;
        }	        
...
{
    if(GlobalUtil::_UseCUDA && GlobalUtil::_NoOpenGLinCUDA) return;
	glEnable(GlobalUtil::_texTarget);
	glActiveTexture(GL_TEXTURE0);
}

void GlobalUtil::InitGLParam(int TargetCUDA)
{
    if(GlobalUtil::_GoodOpenGL == -1) glewInit();
    if(GlobalUtil::_GoodOpenGL == 2) return;
    if(GlobalUtil::_GoodOpenGL == 0) return; 
    if(TargetCUDA && GlobalUtil::_NoOpenGLinCUDA)
    {
    ...
    }else
    {
	    ...
	    if(GlobalUtil::_GoodOpenGL && GlobalUtil::_IsNvidia == 0 && GlobalUtil::_UseGLSL == 0 )
	    {
		    std::cerr << "Switch from CG to GLSL for non-nVidia graphic card \n";
