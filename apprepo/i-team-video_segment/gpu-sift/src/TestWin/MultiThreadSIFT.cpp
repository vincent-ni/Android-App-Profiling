...
public:
    MultiThreadSIFT(int device_id = 0, void* thread_param = NULL)
    {
...
public:
    ...
    virtual void InitializeSIFT()
    {
        ...
        char * argv[] = {"-fo", "-1",  "-v", "0", "-nogl", "-cuda", device};//-nogl is important for multithreading 
...
{
public:
    MultiThreadDemo(int deviceid, char* filename):  MultiThreadSIFT(deviceid, filename)
    {
    }
    virtual void RunTask()
    {
...
{
public:
    MultiProcessDemo(int deviceid, char* filename):  MultiThreadSIFT(deviceid, filename)
    {
    }	
    virtual void RunTask()
    {
