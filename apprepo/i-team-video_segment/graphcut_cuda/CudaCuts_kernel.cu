...
kernel_push_nonatomic(int *g_left_weight, int *g_right_weight, int *g_down_weight, int *g_up_weight,
		int *g_sink_weight, int *g_push_reser, int *g_pull_left, int *g_pull_right,
		...
		int graph_size, int width, int rows, int graph_size1, int width1, int rows1)
{
	...
	__shared__ int height_fn[340];

	int temp_mult = __umul24(y1+1 , 34 ) + x1 + 1 ;
	...
	int temp_mult2 = 340 + __umul24(y1,32) + x1 ; int flow_push = g_push_reser[thid] ;

	__syncthreads();

	if( thid < graph_size && g_relabel_mask[thid] == 1 && x < width - 1 && x > 0 && y < rows - 1 && y > 0)
	{
		int  min_flow_pushed = 0 ;
		{
			...
			if(temp_weight > 0 && flow_push  > 0 && height_fn[temp_mult] == height_fn[temp_mult + 34] + 1 )
			{
				...
				g_pull_down[thid + width1] = min_flow_pushed ;
				flow_push = flow_push - min_flow_pushed ;
			}else g_pull_down[thid + width1] = -1 ;
			...
			if(temp_weight > 0 && flow_push > 0 && height_fn[temp_mult] == height_fn[temp_mult - 34] + 1)
			{
				...
				g_pull_up[thid - width1] =  min_flow_pushed ;
				flow_push = flow_push - min_flow_pushed ;
			}else g_pull_up[thid - width1] = -1 ;
			g_push_reser[thid] = flow_push ;
		}
	}
...
krelabel_nonatomic( int *g_left_weight, int *g_right_weight, int *g_down_weight, int *g_up_weight,
		int *g_push_reser, int *g_sink_weight, int *g_pull_left, int *g_pull_right, int *g_pull_down, int *g_pull_up, 
		int *g_relabel_mask, int* g_height_read, int* g_height_write, int graph_size,
		int width, int rows, int graph_size1, int width1, int height1)
{
	...
	(threadIdx.x == 31 && x < width1 - 1 ) ? height_fn[temp_mult + 1] =  (g_height_read[thid + 1]) : 0;
	(threadIdx.x == 0  && x > 0 ) ? height_fn[temp_mult - 1] = (g_height_read[thid - 1]) : 0;
	(threadIdx.y == 7  && y < height1 - 1 ) ? height_fn[temp_mult + 34] = (g_height_read[thid + width1]) : 0;
	(threadIdx.y == 0  && y > 0 ) ? height_fn[temp_mult - 34] = (g_height_read[thid - width1]) : 0;
	...
	if(thid < graph_size && x < width && x > 0 && y < rows && y > 0 )
	{
