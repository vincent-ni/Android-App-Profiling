...
TVL1::TVL1() {
    pfactor_ = 0.7;             // scale each level by this amount
    max_plevels_ = MAX_PLEVELS; // number of pyramid levels
    max_iters_ = 7;             // u v w update loop
    lambda_ = 40;               // smoothness constraint
    max_warps_ = 5;             // warping u v warping
    min_img_sz_ = 16;           // min max img in pyramid

    // misc
    plevels_ = max_plevels_;
...
void TVL1::gen_pyramid_sizes(int M, int N) {
    ...
    float sN = N;
    // store resizing
    for (int level = 0; level <= plevels_; ++level) {
        if (level == 0) {
        } else {
            sM *= pfactor_;
            sN *= pfactor_;
        ...
        printf(" pyr %d: %d x %d \n", level, (int)sM, (int)sN);
        if (sM < min_img_sz_ || sN < min_img_sz_) { plevels_ = level; break; }
    }
}
...
void TVL1::tv_l1_dual(cl_mem* p, int level) {
    ...
    for (int j = 0; j < max_warps_; ++j) {
        ...
        sclSetKernelArgs(software[5],
                         ...
                         &m_dx, &m_dy, &m_dt, &m_sq,
                         fargsize, &sigma, fargsize, &tau, fargsize, &lambda_, fargsize, &gamma,
                         iargsize, &pyr_M[level], iargsize, &pyr_N[level]);
...
void TVL1::process_pyramids(float* ou, float* ov) {
    ...
    // pyramid loop
    for (int level = plevels_ - 1; level >= 0; level--) {
...
void TVL1::optical_flow_tvl1(Mat& m_I0, Mat& m_I1, Mat& m_Iu, Mat& m_Iv) {
    ...
    if(m_I0.channels() != 1) cvtColor(m_I0, mgray1, CV_BGR2GRAY);
    else { mgray1 = m_I0; }
    ...
    if(m_I1.channels() != 1) cvtColor(m_I1, mgray2, CV_BGR2GRAY);
    else { mgray2 = m_I1; }
