/* #######################################################################
   ...
   Modified by Chris McClanahan 11/01/11
*/
...
#include "simpleCL.h"
...
#ifdef MAC
        char* flags = "-cl-mad-enable -cl-fast-relaxed-math -DMAC";
#else
        char* flags = "-cl-mad-enable -cl-fast-relaxed-math";
#endif
#ifdef DEBUG
        cl_int err;
        char build_c[4096];
    ...
    cl_event sclSetAndLaunchKernel(sclHard hardware, sclSoft software, size_t* global_work_size, size_t* local_work_size, const char* sizesValues, ...) {
        va_list argList;
        cl_event event;
        va_start(argList, sizesValues);
        _sclVSetKernelArgs(software, sizesValues, argList);
    ...
    cl_event sclSetAndEnqueueKernel(sclHard hardware, sclSoft software, size_t* global_work_size, size_t* local_work_size, const char* sizesValues, ...) {
        va_list argList;
        cl_event event;
        va_start(argList, sizesValues);
        _sclVSetKernelArgs(software, sizesValues, argList);
