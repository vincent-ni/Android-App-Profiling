...
@synthesize img_id=img_id_, scale=scale_, rotation=rotation_, trans_x=trans_x_, trans_y=trans_y_,
image=image_, locked=locked_;
...
- (id) initWithFXImage:(FXImage*)img {
  if (self = [super init]) {
    ...
    scale_ = std::min(bounds.size.width / self.img_width,
                      bounds.size.height / self.img_height) * 0.5;
