...
if( WIN32 )
   ...
   SET( FFMPEG_LIBRARIES "${FFMPEG_LIBRARY_DIR}\\avformat-52.lib"
	 		 "${FFMPEG_LIBRARY_DIR}\\avcodec-52.lib"
			 "${FFMPEG_LIBRARY_DIR}\\avutil-49.lib"
	 		 "${FFMPEG_LIBRARY_DIR}\\avdevice-52.lib")
   SET( FFMPEG_SWS_LIBRARIES "${FFMPEG_LIBRARY_DIR}\\swscale-0.lib" )
   SET( FFMPEG_LIBRARY_DIR $ENV{FFMPEGDIR}\\lib )
   SET( FFMPEG_INCLUDE_PATHS $ENV{FFMPEGDIR}\\include )

   # check to see if we can find swscale
   SET( TMP_ TMP-NOTFOUND )
   FIND_FILE( TMP_ "swscale-0.lib" PATH ${FFMPEG_LIBRARY_DIR})
   IF ( TMP_ )
      SET( SWSCALE_FOUND TRUE )
      message("SWSCALE found and added to FFMPEG_LIBRARIES")
   ENDIF( TMP_ )
else( WIN32 )
...
IF ( NOT WIN32 )
   FOREACH( LIB_ ${FFMPEG_LIBRARIES} )
	 ...
	 SET( FFMPEG_LIBRARIES_FULL ${FFMPEG_LIBRARIES_FULL} ${TMP_} )
      ELSE ( TMP_ )
         MESSAGE("LIBRARY ${LIB_} NOT FOUND!")
      ENDIF ( TMP_ )
      SET( TMP_ TMP-NOTFOUND )
   ENDFOREACH( LIB_ )
...
IF(NOT FFMPEG_FOUND)
  ...
  IF(FFMPEG_FIND_REQUIRED)
    MESSAGE(FATAL_ERROR
            "FFMPEG required but some headers or libs not found. If you are using windows, please specify it's location with FFMPEGDIR env. variable.")
